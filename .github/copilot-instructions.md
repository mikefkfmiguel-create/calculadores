...
A app foi rebrandada de "AVK Calculadores" para "Mike Apps Calculadores" (`index.html`, `ecra-complexo.html`, `manifest.json`, `CLAUDE.md`, título e cabeçalho). O ícone (`icons/mike-logo.png`, favicons, `icon-192/512(-maskable).png`, `apple-touch-icon.png`) é uma reconstrução em SVG feita a partir da especificação de um kit de marca fornecido pelo utilizador (grelha 512, 4 formas — quadrado/squircle/círculo/"play" — a 168px com 32px de intervalo a começar em 72, raios 30/64/círculo, 5 cores hex documentadas no commit) — os ficheiros oficiais do kit (SVG vetorial, `.ico`) nunca chegaram como anexo real a este ambiente, só foram vistos inline numa conversa, por isso o ícone em produção é uma aproximação fiel mas não byte-a-byte igual ao original. Se o utilizador anexar os ficheiros oficiais mais tarde, substituir diretamente.
