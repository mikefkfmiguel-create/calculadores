...
// array global LED_TILES_DATA já preenchido antes de chamar calcLedZones().
  ...
  document.getElementById("lz-add").addEventListener("click", function () { lzAddZone(); });
  ...
  // pelo nome numa lista comprida. O diagrama fica fixo no topo (ver
  // #lz-diagram-card), por isso não basta centrar o card — tem de se
  // descontar a altura do próprio diagrama, senão ele tapa a parte de
  // cima do card para onde se salta.
  function lzJumpToZoneCard(zoneId) {
    ...
    var coverH = (diagramCard && getComputedStyle(diagramCard).position !== "static") ? diagramCard.getBoundingClientRect().height : 0;
    var targetTop = card.getBoundingClientRect().top + window.scrollY - coverH - 20;
